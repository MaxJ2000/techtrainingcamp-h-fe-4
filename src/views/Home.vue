<template>
  <div class="home">
    ...
    <Header>我是上帝</Header>
    <v-container fluid>
      <div class="rm">房间号：<span>{{ getRoomId }}</span></div>
      ...
      <form>
        ...
        <div>
          <label>可选神牌</label>
          <input type="checkbox" value="女巫" @click="change(0)" />女巫<br>
          <input type="checkbox" value="猎人" @click="change(1)" />猎人<br>
          <input type="checkbox" value="预言家" @click="change(2)" />预言家
        </div>
        <div>
          <label>可选模式</label>
          <input type="radio" value="屠边" name="model" @click="radiochange" />屠边<br>
